...
//接收api传来的数据，进行一些逻辑处理
func GetProDetails() (prolist []map[string]int,prolistPlus []map[string]string,err error) {
	...
	projectDetails,err := dao.GetProDetailsDao()
	for _,project := range projectDetails{
		demandId := project.DemandId
		onePro,err = dao.GetOneProDetailsDao(demandId)
